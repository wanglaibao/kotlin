...
class Kapt3GradleSubplugin @Inject internal constructor(private val registry: ToolingModelBuilderRegistry) :
    ...
    private fun Kapt3SubpluginContext.registerSubpluginOptions(
        ...
        taskProvider.configure { taskInstance ->
            ...
            // FIXME?
//            project.runOnceAfterEvaluated("add kapt compiler plugin options to the compile task", taskProvider) {
...
//            }
    ...
    private fun Kapt3SubpluginContext.createKaptKotlinTask(useWorkerApi: Boolean): TaskProvider<out KaptTask> {
        ...
        if (javaCompile != null) {
            val androidVariantData = KaptWithAndroid.androidVariantData(this)
            if (androidVariantData != null) {
                KaptWithAndroid.registerGeneratedJavaSource(this, project, androidVariantData, kaptTaskProvider)
            } else {
...
// Don't reference the BaseVariant type in the Kapt plugin signature, as those type references will fail to link when there's no Android
// Gradle plugin on the project's plugin classpath
private object KaptWithAndroid {
    // To avoid loading the BaseVariant type and instead lazily load it when evaluation reaches it in the body, inline the function:
    ...
    @Suppress("NOTHING_TO_INLINE")
    // To avoid loading the BaseVariant type and instead lazily load it when evaluation reaches it in the body, inline the function:
    inline fun registerGeneratedJavaSource(
